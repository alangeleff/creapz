...
const FUSION_PRAY_IMG=new Image(); FUSION_PRAY_IMG.src='./assets/fus_pray.png?v='+ASSET_VER;
...
function prayImg(){ const im=chosen==='fusion'?FUSION_PRAY_IMG:chosen==='son'?SON_PRAY_IMG:(isDing(chosen)?DINGBAT_PRAY_IMG:CREAPER_PRAY_IMG); return (im&&im.complete&&im.naturalWidth)?im:null; }
function drawPrayFrame(sx,yy,fc,scl,foot,atk){ const pimg=(atk && !isDing(chosen) && chosen!=='son' && chosen!=='fusion' && CREAPER_PRAY_CHIBI_IMG.complete && CREAPER_PRAY_CHIBI_IMG.naturalWidth)?CREAPER_PRAY_CHIBI_IMG:prayImg(); if(!pimg) return; const hh=(chosen==='fusion'?150:chosen==='son'?132:isDing(chosen)?107:142)*(scl||1), ww=hh*pimg.naturalWidth/pimg.naturalHeight; ctx.save(); ctx.imageSmoothingEnabled=true; if(fc<0){ ctx.translate(sx,0); ctx.scale(-1,1); ctx.translate(-sx,0);} ctx.drawImage(pimg, sx-ww/2, yy-hh+(foot===undefined?14:foot), ww, hh); ctx.restore(); }
const POWER_IMGS={};
...
// In-play hot-swap between cReaper-family and Dingbat-family (top diamond btn / KeyC). Position+momentum stay on p.
function swapChar(){
  if(mode!=='play'||!p||p.dead||p.won||p.winning) return;
  if(chosen==='fusion'||chosen==='son') return;            // special forms don't pair-swap
  chosen = isDing(chosen) ? (creaperSkin||'default') : (dingSkin||'dingbat');
  if(isDing(chosen)) dingSkin=chosen; else creaperSkin=chosen;
  p.attackT=0; p.castT=0; p.castFired=true; p.diveT=0; p.slamT=0;   // cancel transients so frame indices stay valid
...
const ETHTILE_IMG=new Image(); ETHTILE_IMG.src='./assets/eth_ground_tile.png?v='+ASSET_VER;
const ETHTOP_IMG=new Image(); ETHTOP_IMG.src='./assets/eth_ground_top.png?v='+ASSET_VER;
...
function corder(){ return (typeof devMode!=='undefined'&&devMode)?ORDER.concat(['son','fusion']):ORDER; }
...
bindBtn('bDB','Space'); bindBtn('bDR','KeyX'); bindBtn('bDL','KeyZ'); bindBtn('bDT','KeyC');   // diamond: bottom=jump, right=bolt, left=melee, top=swap character
...
const FZK = { zombie:FZ, zgen:FZ, gob:{idle:13, walk:12, attack:43}, bd:{idle:12, walk:12, attack:12}, golem:{idle:12, walk:12, attack:18}, witch:{idle:12, jump:22, attack:12}, skel:{idle:24, walk:24, run:24, jump:24, attack:24, punch:24}, knight:{idle:12, walk:12, jump:12, attack:17}, angel:{idle:24, walk:24, run:24, attack:24} };
...
function reset(keep){
  ...
  const sy = keep && p && p.spawnY!==undefined ? p.spawnY : (ST&&ST.spawnY!==undefined ? ST.spawnY : (segFloorsAt(sx)[0]!==undefined?segFloorsAt(sx)[0]:GROUND));
...
function polyCross(x){ const out=[];
  for(const ob of POLY.objs){ if(!polyCollides(ob)||ob.line) continue; const S=ob.samples;
...
function polyBlock(nx){ const dir=Math.sign(nx-p.x)||p.facing; const ex=nx+dir*(PW/2); const run=Math.abs(ex-p.x)||1;
  ...
  for(let i=0;i<cs.length;i+=2){ if(cs[i].wall||cs[i].wn==='oneway') continue; const d=Math.abs(cs[i].y-p.y); if(d<best){ best=d; surf=cs[i].y; } }
...
function polyLineSurfaces(x){ const out=[];
  for(const ob of POLY.objs){ if(!ob.line || !polyCollides(ob)) continue; const S=ob.samples;
    for(let i=1;i<S.length;i++){ const a=S[i-1],b=S[i]; if((a.x<=x&&x<b.x)||(b.x<=x&&x<a.x)){ if((a.wn||'auto')==='pass') continue; const t=(x-a.x)/((b.x-a.x)||1e-6); out.push({y:a.y+(b.y-a.y)*t, wn:a.wn||'auto'}); } } }
  return out; }
function resolvePoly(prevFeet){
  if(p.flying||p.dead||p.won||p.winning||!POLY) return;
  if(p.vy>=0){
    let best=null;                                   // highest valid floor to land on
...
function polySingleFill(g,img,S,closed,ox){ if(ox===undefined)ox=camX; let x0=1e9,x1=-1e9,y0=1e9,y1=-1e9; for(const s of S){ if(s.x<x0)x0=s.x; if(s.x>x1)x1=s.x; if(s.y<y0)y0=s.y; if(s.y>y1)y1=s.y; } if(!closed) y1=WORLDH;
  ...
  g.drawImage(img, dx-ox, dy, dw, dh); }
function polyObjFit(g,img,S,ox){ let x0=1e9,x1=-1e9,y0=1e9,y1=-1e9; for(const s of S){ if(s.x<x0)x0=s.x; if(s.x>x1)x1=s.x; if(s.y<y0)y0=s.y; if(s.y>y1)y1=s.y; } g.drawImage(img, x0-ox, y0, x1-x0, y1-y0); }
function _unrotS(S,rot){ if(!rot) return {Sf:S,cx:0,cy:0}; let cx=0,cy=0; for(const p of S){cx+=p.x;cy+=p.y;} cx/=S.length; cy/=S.length; const cs=Math.cos(-rot),sn=Math.sin(-rot); const Sf=S.map(p=>{const dx=p.x-cx,dy=p.y-cy;return{x:cx+dx*cs-dy*sn,y:cy+dx*sn+dy*cs};}); return {Sf,cx,cy}; }
function polyFilterStr(ob){ const br=(ob.br!=null?ob.br:1), sa=(ob.sat!=null?ob.sat:1), hu=(ob.hue||0); if(br===1&&sa===1&&hu===0) return null; return 'brightness('+br+') saturate('+sa+') hue-rotate('+hu+'deg)'; }
function adjTexImg(ob){ const img=ob.tex?polyTexImg(ob.tex):null; if(!img) return null; const ff=polyFilterStr(ob); if(!ff) return img; if(!img.naturalWidth) return img; const key=ob.tex+'|'+ff; if(ob._adjC && ob._adjKey===key) return ob._adjC; const c=document.createElement('canvas'); c.width=img.naturalWidth; c.height=img.naturalHeight; const x=c.getContext('2d'); x.filter=ff; x.drawImage(img,0,0); c.naturalWidth=c.width; c.naturalHeight=c.height; ob._adjC=c; ob._adjKey=key; return c; }
...
function drawFringe(ob,S,alpha){ const img=ob.fringeTex?adjFringeImg(ob):null; if(!img) return; const H=ob.fringeH||44;
...
try{
  const q=new URLSearchParams(location.search);
  if(q.get('polytest')==='1') window.__polyDemo=true;
  if(q.get('test')==='1'){
...
function update(dt){
  ...
  if (p.castT>0){
    ...
    const fireAt = chosen==='dingbat' ? 2 : (chosen==='son'||chosen==='fusion') ? 0 : 3;   // dingbat: bolt leaves on the open-mouth frame (11f @40fps)
  ...
  for (const z of zombies){
    ...
    if (!z.aggro && ad<260 && dy<160 && _onscr && !p.dead){
      ...
      else if (z.kw==='skel') playSfx('sfx_skelsee',0.9);
    ...
    if (z.kw==='angel'){
      ...
      if(!z.aggro && Math.abs(p.x-z.x)<560 && (z.x-camX)>-40 && (z.x-camX)<W+40 && !p.dead) z.aggro=true;
      else if(z.aggro && Math.abs(p.x-z.x)>820) z.aggro=false;
      if(z.diveT>0){
        z.diveT-=dt*efr; z.state='run'; z.x+=z.dvx*dt*efr; z.y+=z.dvy*dt*efr; if(z.hitCd>0) z.hitCd-=dt;
        if(z.hitCd<=0 && p.inv<=0 && !p.dead && overlap(pBodyBox(), zBodyBox(z))){ hurtPlayer(z.x,1); z.hitCd=0.6; }
        z.x=clamp(z.x,20,WORLD-20); z.y=clamp(z.y,70,GROUND);
        if(z.diveT<=0) z.angelState='fly';
        continue;
      }
      if(z.atkT>0){
        z.atkT-=dt*efr; z.state='attack';
        const cp=Math.max(0,Math.min(1,1-z.atkT/z.atkDur));
        z.orbX=z.x+z.facing*50; z.orbY=z.y-60; z.orbR=6+cp*34; z.charging=!z.fired;
        if(cp>=0.82 && !z.fired){ z.fired=true; z.charging=false; angelLaunchOrb(z); }
        if(z.atkT<=0){ z.angelState='fly'; z.charging=false; }
        continue;
      }
      if(z.aggro && !p.dead){
        z.state='walk'; z.orbitPh+=dt*0.85;
        const tx=p.x+Math.cos(z.orbitPh)*215, ty=p.y-150+Math.sin(z.orbitPh)*42;
        z.x+=(tx-z.x)*Math.min(1,dt*1.7); z.y+=(ty-z.y)*Math.min(1,dt*1.7); z.actCd-=dt;
        if(z.actCd<=0){
          if(Math.random()<0.5){ z.atkT=z.atkDur; z.fired=false; z.actCd=2.6+Math.random()*1.4; playSfx('sfx_angelcharge',0.9); }
          else { const dx=p.x-z.x, dy=(p.y-40)-z.y, d=Math.hypot(dx,dy)||1, sp=500; z.dvx=dx/d*sp; z.dvy=dy/d*sp; z.diveT=Math.min(0.8, d/sp+0.18); z.hitCd=0; z.actCd=3+Math.random()*1.6; playSfx('sfx_rwhoosh',0.7); }
        }
      } else { z.state='idle'; z.y+=Math.sin(gt*1.6+z.t)*0.4; }
      z.x=clamp(z.x,20,WORLD-20); z.y=clamp(z.y,70,GROUND);
      continue;
    }
    if (CHASER[z.kw]){
      const C=CHASER[z.kw]; if(z.atkCd>0) z.atkCd-=dt; if(C.wave && z.castCd>0) z.castCd-=dt; const ad=Math.abs(p.x-z.x);
      ...
      if(C.stomp){
        ...
        if(z.stompPhase==='slam'){ z.stompT-=dt; const k=Math.max(0,Math.min(1,1-z.stompT/C.slamDur)); z.y=z.slamY0+(z.sy0-z.slamY0)*(k*k); z.state='jump'; if(z.stompT<=0){ z.y=z.sy0; z.stompPhase='recover'; z.stompT=C.recoverDur; addShake(15,0.55); playSfx('sfx_knightimpact',1.0); for(let i=0;i<30;i++){ const sd=(Math.random()<0.5?-1:1); zbits.push({x:z.x+sd*(12+Math.random()*135), y:z.sy0-2, vx:sd*(70+Math.random()*255), vy:-40-Math.random()*175, sz:2+Math.random()*3.6, life:0.4+Math.random()*0.45, t:0, c:['#8a2a2a','#c0392b','#6a1a1a','#bbb','#7a1818'][(Math.random()*5)|0]}); } if(p.onGround && !p.dead && p.inv<=0 && Math.abs(p.x-z.x)<(C.stompR||100)) hurtPlayer(z.x,C.stompDmg||2); } else if(p.inv<=0&&!p.dead&&overlap(pBodyBox(),zBodyBox(z)))hurtPlayer(z.x,1); continue; }
      ...
      if(C.wave && z.castT>0){
        z.castT-=dt*efr; z.state='punch';
        const cf=Math.min(C.castFrames-1, Math.floor((C.castFrames/C.castFps - z.castT)*C.castFps));
        if(cf>=C.castFire && !z.fired){ z.fired=true; skelFlameWave(z); }
        z.x=terrWallX(z.x,zpx,z.y,16);
        if(z.castT<=0) z.castCd=(C.waveCdMin||2.3)+Math.random()*1.4;
      ...
      if(z.aggro && !p.dead){
        ...
        else if(ad<=C.atkRange && z.atkCd<=0){ z.atkT=C.atkFrames/C.atkFps; z.state='attack'; z.fired=false; if(!C.ranged) playSfx(z.kw==='knight'?'sfx_knightstomp':(z.kw==='skel'?'sfx_skeljump':'sfx_zswing'),0.7); }
...
function drawEthTopper(x0,x1,sgy){
  const img=ETHTOP_IMG; if(!img.complete||!img.naturalWidth) return;
  const TH=64, tw=TH*img.naturalWidth/img.naturalHeight, topY=sgy-0.091*TH;
  ctx.save(); ctx.beginPath(); ctx.rect(x0,topY,x1-x0,TH); ctx.clip(); ctx.imageSmoothingEnabled=true;
  const off=((camX%tw)+tw)%tw;
  for(let x=-off; x<W+tw; x+=tw) ctx.drawImage(img,x,topY,tw,TH);
  ctx.restore();
...
function drawOneTerrace(s){
  const crypt=(ST.theme==='crypt'), kind=(s[6]||'cemetery');
  {
    ...
    } else if (kind==='ethereal' && ETHTILE_IMG.complete && ETHTILE_IMG.naturalWidth){
      tileImage(ETHTILE_IMG, sgy, bot-sgy, 1);
    } else {
    ...
    else if (kind==='ethereal') drawEthTopper(x0,x1,sgy);
    if (SPR.grass && !crypt && kind!=='cave' && kind!=='witchwood' && kind!=='ethereal'){
...
const SKINC={default:'#5b3fd6', green:'#18a88a', blue:'#d82a2a', red:'#20c8c8', wraith:'#ff7a2a', gilded:'#2fcf55', bone:'#8a2a6a', crimson:'#d8e040', son:'#3aa0e0', fusion:'#9a4ae0'};
...
function angelBlast(z){
  ...
  for(let i=0;i<10;i++) zbits.push({x:wx,y:wy,vx:(Math.random()-0.5)*120,vy:(Math.random()-0.5)*120,sz:1.5+Math.random()*2,life:0.3+Math.random()*0.25,t:0,c:['#b06bff','#7a3ad8','#e0c0ff'][(Math.random()*3)|0]});
}
...
function skelFlameWave(z){
  ...
  for(let i=0;i<3;i++){ flameWaves.push({x:z.x+dir*34, y:z.y-6, dir, dist:0, reach:470, spd:305, delay:i*0.17, t:0, dead:false, parts:[], r:12+i}); playSfx('sfx_skelfire',0.85,i*0.17); }
}
...
function drawZombie(z){
  ...
  else if (z.kw==='angel'){ if(state==='attack') fi=Math.min(a.frames-1, Math.floor((z.atkDur-z.atkT)*24)); else fi=Math.floor(z.t*24)%a.frames; }
  else if (CHASER[z.kw]){ const _C=CHASER[z.kw]; if(state==='attack') fi=Math.min(a.frames-1, Math.floor((_C.atkFrames/_C.atkFps - z.atkT)*_C.atkFps)); else if(state==='punch') fi=Math.min(a.frames-1, Math.floor((_C.castFrames/_C.castFps - z.castT)*_C.castFps)); else fi=Math.floor(z.t*FZK[z.kw][state])%a.frames; }
...
function drawPlayerLayer(){
  if (p.won) return;
  const sx=p.x-camX;
  { const _sf=p.swapFlash?(performance.now()-p.swapFlash):1e9; if(_sf<300){ const k=_sf/300; ctx.save(); ctx.globalAlpha=(1-k)*0.85; ctx.strokeStyle='rgba(222,182,255,1)'; ctx.lineWidth=3; ctx.beginPath(); ctx.arc(sx,p.y-46,Math.max(0,12+k*56),0,Math.PI*2); ctx.stroke(); ctx.restore(); } }
  ...
  if (!p.dead){
    if (p.slamT>0){
      ...
      if(_upcast){ _skip=true; const fast=Math.floor(gt*24)%2; ctx.filter= fast ? 'grayscale(1) invert(1) brightness(1.15)' : 'grayscale(1) brightness(1.7) contrast(1.25)'; drawPrayFrame(sx,p.y,p.facing,(chosen==='son'||chosen==='fusion')?1.0:(isDing(chosen)?1.05:0.82),isDing(chosen)?-1:2,true); ctx.filter='none'; }
