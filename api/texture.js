...
function characterPrompt(prompt, style){ return `A single full-body 2D video-game character/monster: ${prompt}. Centered, entire body visible head-to-toe, neutral standing idle pose, side view facing right (platformer sprite). The ENTIRE background is pure solid magenta #FF00FF; the character contains NO magenta/pink/purple. Stylized 2D side-scrolling game art, flat even lighting, no cast shadow, no ground, no scene, no text.`+(style?` Art style: ${style}.`:''); }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const mode = (body.mode === 'fringe') ? 'fringe' : (body.mode === 'object' ? 'object' : (body.mode === 'character' ? 'character' : (body.mode === 'upscale' ? 'upscale' : (body.mode === 'edit' ? 'edit' : (body.mode === 'pose' ? 'pose' : 'texture')))));
    const styleRefs = Array.isArray(body.styleRefs) ? body.styleRefs.filter(x => typeof x === 'string' && x.length > 100).slice(0, 2) : [];
    if (mode === 'pose') { if (typeof body.image !== 'string' || body.image.length < 200 || !prompt) return res.status(400).json({ error: 'image + prompt required' }); const po = await genGeminiI2I('The image shows a 2D side-scroller game character on a magenta background. Redraw the SAME character — identical design, colors, outfit, proportions, art style, line weight, shading, and SATURATION — in this new pose: ' + prompt + '. Render it FULLY PAINTED, SOLID and OPAQUE, with the exact same rich colors, contrast, and detail as the original — do NOT output a faint sketch, pencil/line-art, washed-out, pale, or semi-transparent version. Include the FULL body with every limb, chain, and detail intact and in frame — nothing cropped, faded, or omitted. Side view facing right, same scale and centering, on a pure solid magenta #FF00FF background. ONLY the pose and limb positions change; the character identity, rendering quality, and color saturation stay exactly the same.', body.image); if (po.err) return res.status(po.status || 500).json({ error: po.err, mode: 'pose' }); return res.status(200).json({ imageBase64: po.imageBase64, mimeType: po.mimeType, mode: 'pose' }); }
    ...
    if (mode === 'character') {
      const full = characterPrompt(prompt, style);
